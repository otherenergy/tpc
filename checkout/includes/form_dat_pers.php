...
include_once ( dirname( dirname(__DIR__) ). '/config/db_connect.php');
include_once('../../assets/lib/class.carrito.php');
include_once('../../assets/lib/funciones.php');
include_once( dirname( dirname(__DIR__) ). '/class/userClass.php');
include_once( dirname( dirname(__DIR__) ). '/class/checkoutClass.php');
...
if ( $_POST['type'] == 'edit') {
	$id = $_POST['id'];
	$reg = $checkoutp->obten_datos_user( $id )[0];
}
...
<div class="row form-modal">
	...
	<script>
		...
		function compruebaDatosUser () {
			...
			// else if($('#input_nif').val().length < 7 ) {
			// 	muestraMensajeLn(`<?php echo $vocabulario_documento_identidad_no_correcto; ?>`);
			// 	return false;
