...
$displayStyle = (($precioTotal < 3000) && $_SESSION['smart_user']['tipo_factura'] == 1) ? 'none' : 'block';

$inputValue = ($precioTotal < 3000 && $_SESSION['smart_user']['tipo_factura'] == 1) ? '' : (!empty($reg->nif) ? $reg->nif : '');
...
<div class="row form-modal">
	<form id="form-datos" class="form_dir_facturacion" action='#' method='POST'>
		<div class="row">
			...
			<div class="form-group col-md-6 DNI" style="display: <?php echo $displayStyle; ?>;">
				<label for="form_nif"><?php echo $vocabulario_dni_nif_nie; ?> <span style="color:red;">*</span></label>
				<input type="text" class="form-control" id="input_nif" name="input_nif" value="<?php echo $inputValue; ?>">
			</div>
			<div class="form-group col-md-6">
...
<script>
	...
	function compruebaDatosEnvio() {
		...
		if ($('.DNI').css('display') !== 'none') {
			if ($('#input_nom').val() == '' || $('#input_ape').val() == '' || $('#input_nif').val() == '') {
				muestraMensajeLn(`<?php echo $vocabulario_nombre_apellido_cif_vacios ?>`);
				return false;
			}
		} else {
			if ($('#input_nom').val() == '' || $('#input_ape').val() == '') {
				muestraMensajeLn(`<?php echo $vocabulario_nombre_apellidos_no_pueden_estar_vacios ?>`);
				return false;
			}
		}
		
		if ($('#input_dir').val() == '' || $('#input_cod').val() == '' || $('#input_pob').val() == '' || $('#input_telf').val() == '') {
