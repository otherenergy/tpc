...
class Carrito
{
	...
	public function __construct()
	{
		...
		if(!isset($_SESSION["carrito"]))
		{
			...
			// $this->carrito["articulo_suelta"] = 0;
			// $this->carrito["articulo_caja"] = 0;
		}
	...
	public function add($articulo = array())
	{
	    ...
	    // if($articulo["cantidad"]>=$articulo["cantidad_caja"]) {
	    	$articulo["total"] = $articulo["cantidad"] * $articulo["precio"];
	    // }else {
	    // 	$articulo["total"] = (floor($articulo["cantidad"]/$articulo["cantidad_caja"])*$articulo['precio_caja']) + (floor($articulo["cantidad"]%$articulo["cantidad_caja"])*$articulo['precio']);
	    // }
	...
	private function update_precio_cantidad()
	{
		...
		foreach ($this->carrito as $row)
		{
			...
			// if($row['cantidad_caja']!=0){
			// 	$precio += ($row['precio_caja'] * floor($row['cantidad']/$row['cantidad_caja']));
			// }
			$precio += ($row['precio'] * (int)@floor($row['cantidad']));
			// $precio += ($row['precio'] * (int)floor(fmod($row['cantidad'],$row['cantidad_caja'])));
			// $precio += ($row['precio'] * floor($row['cantidad']%$row['cantidad_caja']));
		...
		// $_SESSION['carrito']["precio_total"] = 0.00;
		$_SESSION['carrito']["precio_total"] = $precio;

		//refrescamos él contenido del carrito para que quede actualizado
		$this->update_carrito();
	}
	...
	public function marca_imprescindible($unique_id, $estado) {
		$_SESSION["carrito"][$unique_id]['imprescindible']=$estado;
	}

	public function actualiza_cantidad($unique_id, $cantidad) {
		$_SESSION["carrito"][$unique_id]['cantidad']=$cantidad;
		$this->update_carrito();
		$this->update_precio_cantidad();
	}

	public function del_imprescindibles() {
		foreach ($this->carrito as $row)
		{
			$row['imprescindible']='0';
		}
	}
