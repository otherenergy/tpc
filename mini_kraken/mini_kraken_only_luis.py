...
mydb = mysql.connector.connect(host="91.134.186.41", user="smartcret_dev", password="8caMDgdJH@", database="smartcret_dev")
...
def crear_carpeta(idioma):
    try:
        ...
        #shutil.copytree("plantilla_smartcret", file_path+base_url+'/')
    except OSError as e:
...
def modificar_archivo(id_tipo,url,id_url,idioma,id_idioma):
    print('url:',url,'id_url:',id_url,'idioma:',idioma,'id_idioma:',id_idioma)
    try:
        if url != '/':
            if id_tipo == 6:
...
for x in idiomas:
  ...
  consulta_urls_cursor="SELECT * FROM urls WHERE id_idioma=%s"
  id_idioma=(id_idioma,)
  ...
  for y in urls:
    ...
    modificar_archivo(id_tipo,url,id_url,idioma,id_idioma)