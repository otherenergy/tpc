	...
	<div class="row">
		...
		<div class="col-md-9">
			...
			<h1 class="center">
				<?php if ( !isset( $_GET['v'] ) )  {
					echo "Últimos " . $pagina;?>
					<button class="print_btn lista" onclick="window.location.href='./pedidos?v=todos'">Ver todos<i class="fa fa-list"></i></button>
				<?php }

			if ( isset( $_GET['v'] ) && $_GET['v'] == 'todos' )  {
					echo $pagina; ?>
					<button class="print_btn lista" onclick="window.location.href='./pedidos'">Ultimos pedidos<i class="fa fa-list"></i></button>
			<?php } ?>
...
<!-- 								<?php if ( $reg->identificador ) {
											echo '<a class="so_books" href="https://books.topciment.com/app/637820086#/salesorders/' . $reg->identificador . '?filter_by=Status.All&per_page=200&sort_column=created_time&sort_order=D' . $reg->fecha_entrega . '" target="_blank">' . $reg->ref_pedido . '</a>';
											}else {
												echo $reg->ref_pedido;
											} ?> -->
										...
										<td class="izq prec">
											...
											/*$total_pagar = $reg->total_sinenvio + $reg->gastos_envio - $reg->importe_descuento + $reg->descuento_iva;

											if ( ( $reg->tipo_impuesto == 2 || $reg->tipo_impuesto == 3 || $reg->tipo_impuesto == 6 ) && $reg->descuento_iva == 0 ) {

												if ( $envio->pais == 'US') {
													echo formatea_importe( $total_pagar ) . '€';
													echo "<br>";
													...
													echo formatea_importe( $total_pagar - $importe_descuento_iva ) . '€';
												}
