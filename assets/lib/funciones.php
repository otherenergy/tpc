...
require ( __DIR__  . '/traducciones.php' );

/**
Recogida de datos de usuario cada vez que carga una página.
Guardamos IP, id si está logueado en web, idioma, url actual, url_previa, fecha y hora
**/
guarda_datos_usuario_web();
...
// 	global $mysqli;
// 	$sql = "SELECT * FROM datos_envio WHERE id_cliente = $id_cliente AND predeterminado = 1 LIMIT 1";
// 	$res = $mysqli->query($sql);
// 	if( mysqli_num_rows ( $res ) > 0) {
// 		return $mysqli->query($sql)->fetch_object()->id;
// 	}
...
function actualiza_envio_pedido ( $id_pedido, $estado_envio, $fecha_envio, $transportista ) {
	global $mysqli;
	$sql = "UPDATE pedidos SET estado_envio='$estado_envio', fecha_envio='$fecha_envio', transportista='$transportista'  WHERE id = $id_pedido";
	return $mysqli->query($sql);
}
...
require ( __DIR__ . '/calcula_transporte.php' );
...
function envia_email () {
	$nombre='Javier';
  $msg = 'Este es un envío de prueba';
	include_once('../../mailings/prueba.php');
}

function envia_email_registro ( $nombre, $email ) {
	include_once('../../mailings/email_registro.php');
}

function envia_email_procesando_pedido ( $nombre, $email, $ref_pedido ) {
	$detalle_pedido = lista_pedido_ref ( $ref_pedido );
	include_once ( './mailings/email_procesando_pedido.php');
}

function envia_email_pedido_recibido () {
	include_once( '../../mailings/email_pedido_recibido.php' );
}
function envia_email_pedido_espera ( $nombre, $email, $ref_pedido ) {
	$detalle_pedido = lista_pedido_ref ( $ref_pedido );
	include_once( './mailings/email_pedido_espera.php' );
}

function envia_email_pago_rechazado ( $nombre, $email ) {
	include_once( './mailings/email_pago_rechazado.php' );
}

function envia_email_recupera_pass ( $email, $pass, $nombre ) {
	include_once( '../../mailings/email_recupera_pass.php' );
}

function envia_email_cambio_pass ( $email, $pass, $nombre ) {
	include_once( '../../mailings/email_cambio_pass.php' );
}

function envia_email_bienvenido_newsletter ( $email ) {
	include_once( '../../mailings/email_bienvenido_newsletter.php' );
}

function envia_email_suscripcion_news_regalo ( $nombre, $email, $lang ) {
	include_once( dirname ( dirname ( __DIR__ ) ) . '/mailings/email_suscripcion_news_regalo.php' );
}
